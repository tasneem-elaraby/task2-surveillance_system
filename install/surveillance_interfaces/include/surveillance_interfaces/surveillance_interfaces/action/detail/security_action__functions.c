// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from surveillance_interfaces:action/SecurityAction.idl
// generated code does not contain a copyright notice
#include "surveillance_interfaces/action/detail/security_action__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `event_type`
// Member `severity`
// Member `description`
#include "rosidl_runtime_c/string_functions.h"

bool
surveillance_interfaces__action__SecurityAction_Goal__init(surveillance_interfaces__action__SecurityAction_Goal * msg)
{
  if (!msg) {
    return false;
  }
  // event_type
  if (!rosidl_runtime_c__String__init(&msg->event_type)) {
    surveillance_interfaces__action__SecurityAction_Goal__fini(msg);
    return false;
  }
  // severity
  if (!rosidl_runtime_c__String__init(&msg->severity)) {
    surveillance_interfaces__action__SecurityAction_Goal__fini(msg);
    return false;
  }
  // description
  if (!rosidl_runtime_c__String__init(&msg->description)) {
    surveillance_interfaces__action__SecurityAction_Goal__fini(msg);
    return false;
  }
  return true;
}

void
surveillance_interfaces__action__SecurityAction_Goal__fini(surveillance_interfaces__action__SecurityAction_Goal * msg)
{
  if (!msg) {
    return;
  }
  // event_type
  rosidl_runtime_c__String__fini(&msg->event_type);
  // severity
  rosidl_runtime_c__String__fini(&msg->severity);
  // description
  rosidl_runtime_c__String__fini(&msg->description);
}

bool
surveillance_interfaces__action__SecurityAction_Goal__are_equal(const surveillance_interfaces__action__SecurityAction_Goal * lhs, const surveillance_interfaces__action__SecurityAction_Goal * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // event_type
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->event_type), &(rhs->event_type)))
  {
    return false;
  }
  // severity
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->severity), &(rhs->severity)))
  {
    return false;
  }
  // description
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->description), &(rhs->description)))
  {
    return false;
  }
  return true;
}

bool
surveillance_interfaces__action__SecurityAction_Goal__copy(
  const surveillance_interfaces__action__SecurityAction_Goal * input,
  surveillance_interfaces__action__SecurityAction_Goal * output)
{
  if (!input || !output) {
    return false;
  }
  // event_type
  if (!rosidl_runtime_c__String__copy(
      &(input->event_type), &(output->event_type)))
  {
    return false;
  }
  // severity
  if (!rosidl_runtime_c__String__copy(
      &(input->severity), &(output->severity)))
  {
    return false;
  }
  // description
  if (!rosidl_runtime_c__String__copy(
      &(input->description), &(output->description)))
  {
    return false;
  }
  return true;
}

surveillance_interfaces__action__SecurityAction_Goal *
surveillance_interfaces__action__SecurityAction_Goal__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_Goal * msg = (surveillance_interfaces__action__SecurityAction_Goal *)allocator.allocate(sizeof(surveillance_interfaces__action__SecurityAction_Goal), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(surveillance_interfaces__action__SecurityAction_Goal));
  bool success = surveillance_interfaces__action__SecurityAction_Goal__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
surveillance_interfaces__action__SecurityAction_Goal__destroy(surveillance_interfaces__action__SecurityAction_Goal * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    surveillance_interfaces__action__SecurityAction_Goal__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
surveillance_interfaces__action__SecurityAction_Goal__Sequence__init(surveillance_interfaces__action__SecurityAction_Goal__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_Goal * data = NULL;

  if (size) {
    data = (surveillance_interfaces__action__SecurityAction_Goal *)allocator.zero_allocate(size, sizeof(surveillance_interfaces__action__SecurityAction_Goal), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = surveillance_interfaces__action__SecurityAction_Goal__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        surveillance_interfaces__action__SecurityAction_Goal__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
surveillance_interfaces__action__SecurityAction_Goal__Sequence__fini(surveillance_interfaces__action__SecurityAction_Goal__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      surveillance_interfaces__action__SecurityAction_Goal__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

surveillance_interfaces__action__SecurityAction_Goal__Sequence *
surveillance_interfaces__action__SecurityAction_Goal__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_Goal__Sequence * array = (surveillance_interfaces__action__SecurityAction_Goal__Sequence *)allocator.allocate(sizeof(surveillance_interfaces__action__SecurityAction_Goal__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = surveillance_interfaces__action__SecurityAction_Goal__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
surveillance_interfaces__action__SecurityAction_Goal__Sequence__destroy(surveillance_interfaces__action__SecurityAction_Goal__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    surveillance_interfaces__action__SecurityAction_Goal__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
surveillance_interfaces__action__SecurityAction_Goal__Sequence__are_equal(const surveillance_interfaces__action__SecurityAction_Goal__Sequence * lhs, const surveillance_interfaces__action__SecurityAction_Goal__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!surveillance_interfaces__action__SecurityAction_Goal__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
surveillance_interfaces__action__SecurityAction_Goal__Sequence__copy(
  const surveillance_interfaces__action__SecurityAction_Goal__Sequence * input,
  surveillance_interfaces__action__SecurityAction_Goal__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(surveillance_interfaces__action__SecurityAction_Goal);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    surveillance_interfaces__action__SecurityAction_Goal * data =
      (surveillance_interfaces__action__SecurityAction_Goal *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!surveillance_interfaces__action__SecurityAction_Goal__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          surveillance_interfaces__action__SecurityAction_Goal__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!surveillance_interfaces__action__SecurityAction_Goal__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `response_summary`
// already included above
// #include "rosidl_runtime_c/string_functions.h"

bool
surveillance_interfaces__action__SecurityAction_Result__init(surveillance_interfaces__action__SecurityAction_Result * msg)
{
  if (!msg) {
    return false;
  }
  // success
  // response_summary
  if (!rosidl_runtime_c__String__init(&msg->response_summary)) {
    surveillance_interfaces__action__SecurityAction_Result__fini(msg);
    return false;
  }
  return true;
}

void
surveillance_interfaces__action__SecurityAction_Result__fini(surveillance_interfaces__action__SecurityAction_Result * msg)
{
  if (!msg) {
    return;
  }
  // success
  // response_summary
  rosidl_runtime_c__String__fini(&msg->response_summary);
}

bool
surveillance_interfaces__action__SecurityAction_Result__are_equal(const surveillance_interfaces__action__SecurityAction_Result * lhs, const surveillance_interfaces__action__SecurityAction_Result * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // success
  if (lhs->success != rhs->success) {
    return false;
  }
  // response_summary
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->response_summary), &(rhs->response_summary)))
  {
    return false;
  }
  return true;
}

bool
surveillance_interfaces__action__SecurityAction_Result__copy(
  const surveillance_interfaces__action__SecurityAction_Result * input,
  surveillance_interfaces__action__SecurityAction_Result * output)
{
  if (!input || !output) {
    return false;
  }
  // success
  output->success = input->success;
  // response_summary
  if (!rosidl_runtime_c__String__copy(
      &(input->response_summary), &(output->response_summary)))
  {
    return false;
  }
  return true;
}

surveillance_interfaces__action__SecurityAction_Result *
surveillance_interfaces__action__SecurityAction_Result__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_Result * msg = (surveillance_interfaces__action__SecurityAction_Result *)allocator.allocate(sizeof(surveillance_interfaces__action__SecurityAction_Result), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(surveillance_interfaces__action__SecurityAction_Result));
  bool success = surveillance_interfaces__action__SecurityAction_Result__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
surveillance_interfaces__action__SecurityAction_Result__destroy(surveillance_interfaces__action__SecurityAction_Result * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    surveillance_interfaces__action__SecurityAction_Result__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
surveillance_interfaces__action__SecurityAction_Result__Sequence__init(surveillance_interfaces__action__SecurityAction_Result__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_Result * data = NULL;

  if (size) {
    data = (surveillance_interfaces__action__SecurityAction_Result *)allocator.zero_allocate(size, sizeof(surveillance_interfaces__action__SecurityAction_Result), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = surveillance_interfaces__action__SecurityAction_Result__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        surveillance_interfaces__action__SecurityAction_Result__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
surveillance_interfaces__action__SecurityAction_Result__Sequence__fini(surveillance_interfaces__action__SecurityAction_Result__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      surveillance_interfaces__action__SecurityAction_Result__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

surveillance_interfaces__action__SecurityAction_Result__Sequence *
surveillance_interfaces__action__SecurityAction_Result__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_Result__Sequence * array = (surveillance_interfaces__action__SecurityAction_Result__Sequence *)allocator.allocate(sizeof(surveillance_interfaces__action__SecurityAction_Result__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = surveillance_interfaces__action__SecurityAction_Result__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
surveillance_interfaces__action__SecurityAction_Result__Sequence__destroy(surveillance_interfaces__action__SecurityAction_Result__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    surveillance_interfaces__action__SecurityAction_Result__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
surveillance_interfaces__action__SecurityAction_Result__Sequence__are_equal(const surveillance_interfaces__action__SecurityAction_Result__Sequence * lhs, const surveillance_interfaces__action__SecurityAction_Result__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!surveillance_interfaces__action__SecurityAction_Result__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
surveillance_interfaces__action__SecurityAction_Result__Sequence__copy(
  const surveillance_interfaces__action__SecurityAction_Result__Sequence * input,
  surveillance_interfaces__action__SecurityAction_Result__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(surveillance_interfaces__action__SecurityAction_Result);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    surveillance_interfaces__action__SecurityAction_Result * data =
      (surveillance_interfaces__action__SecurityAction_Result *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!surveillance_interfaces__action__SecurityAction_Result__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          surveillance_interfaces__action__SecurityAction_Result__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!surveillance_interfaces__action__SecurityAction_Result__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `current_step`
// already included above
// #include "rosidl_runtime_c/string_functions.h"

bool
surveillance_interfaces__action__SecurityAction_Feedback__init(surveillance_interfaces__action__SecurityAction_Feedback * msg)
{
  if (!msg) {
    return false;
  }
  // current_step
  if (!rosidl_runtime_c__String__init(&msg->current_step)) {
    surveillance_interfaces__action__SecurityAction_Feedback__fini(msg);
    return false;
  }
  // progress_percent
  return true;
}

void
surveillance_interfaces__action__SecurityAction_Feedback__fini(surveillance_interfaces__action__SecurityAction_Feedback * msg)
{
  if (!msg) {
    return;
  }
  // current_step
  rosidl_runtime_c__String__fini(&msg->current_step);
  // progress_percent
}

bool
surveillance_interfaces__action__SecurityAction_Feedback__are_equal(const surveillance_interfaces__action__SecurityAction_Feedback * lhs, const surveillance_interfaces__action__SecurityAction_Feedback * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // current_step
  if (!rosidl_runtime_c__String__are_equal(
      &(lhs->current_step), &(rhs->current_step)))
  {
    return false;
  }
  // progress_percent
  if (lhs->progress_percent != rhs->progress_percent) {
    return false;
  }
  return true;
}

bool
surveillance_interfaces__action__SecurityAction_Feedback__copy(
  const surveillance_interfaces__action__SecurityAction_Feedback * input,
  surveillance_interfaces__action__SecurityAction_Feedback * output)
{
  if (!input || !output) {
    return false;
  }
  // current_step
  if (!rosidl_runtime_c__String__copy(
      &(input->current_step), &(output->current_step)))
  {
    return false;
  }
  // progress_percent
  output->progress_percent = input->progress_percent;
  return true;
}

surveillance_interfaces__action__SecurityAction_Feedback *
surveillance_interfaces__action__SecurityAction_Feedback__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_Feedback * msg = (surveillance_interfaces__action__SecurityAction_Feedback *)allocator.allocate(sizeof(surveillance_interfaces__action__SecurityAction_Feedback), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(surveillance_interfaces__action__SecurityAction_Feedback));
  bool success = surveillance_interfaces__action__SecurityAction_Feedback__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
surveillance_interfaces__action__SecurityAction_Feedback__destroy(surveillance_interfaces__action__SecurityAction_Feedback * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    surveillance_interfaces__action__SecurityAction_Feedback__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
surveillance_interfaces__action__SecurityAction_Feedback__Sequence__init(surveillance_interfaces__action__SecurityAction_Feedback__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_Feedback * data = NULL;

  if (size) {
    data = (surveillance_interfaces__action__SecurityAction_Feedback *)allocator.zero_allocate(size, sizeof(surveillance_interfaces__action__SecurityAction_Feedback), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = surveillance_interfaces__action__SecurityAction_Feedback__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        surveillance_interfaces__action__SecurityAction_Feedback__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
surveillance_interfaces__action__SecurityAction_Feedback__Sequence__fini(surveillance_interfaces__action__SecurityAction_Feedback__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      surveillance_interfaces__action__SecurityAction_Feedback__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

surveillance_interfaces__action__SecurityAction_Feedback__Sequence *
surveillance_interfaces__action__SecurityAction_Feedback__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_Feedback__Sequence * array = (surveillance_interfaces__action__SecurityAction_Feedback__Sequence *)allocator.allocate(sizeof(surveillance_interfaces__action__SecurityAction_Feedback__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = surveillance_interfaces__action__SecurityAction_Feedback__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
surveillance_interfaces__action__SecurityAction_Feedback__Sequence__destroy(surveillance_interfaces__action__SecurityAction_Feedback__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    surveillance_interfaces__action__SecurityAction_Feedback__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
surveillance_interfaces__action__SecurityAction_Feedback__Sequence__are_equal(const surveillance_interfaces__action__SecurityAction_Feedback__Sequence * lhs, const surveillance_interfaces__action__SecurityAction_Feedback__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!surveillance_interfaces__action__SecurityAction_Feedback__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
surveillance_interfaces__action__SecurityAction_Feedback__Sequence__copy(
  const surveillance_interfaces__action__SecurityAction_Feedback__Sequence * input,
  surveillance_interfaces__action__SecurityAction_Feedback__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(surveillance_interfaces__action__SecurityAction_Feedback);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    surveillance_interfaces__action__SecurityAction_Feedback * data =
      (surveillance_interfaces__action__SecurityAction_Feedback *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!surveillance_interfaces__action__SecurityAction_Feedback__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          surveillance_interfaces__action__SecurityAction_Feedback__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!surveillance_interfaces__action__SecurityAction_Feedback__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
#include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `goal`
// already included above
// #include "surveillance_interfaces/action/detail/security_action__functions.h"

bool
surveillance_interfaces__action__SecurityAction_SendGoal_Request__init(surveillance_interfaces__action__SecurityAction_SendGoal_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    surveillance_interfaces__action__SecurityAction_SendGoal_Request__fini(msg);
    return false;
  }
  // goal
  if (!surveillance_interfaces__action__SecurityAction_Goal__init(&msg->goal)) {
    surveillance_interfaces__action__SecurityAction_SendGoal_Request__fini(msg);
    return false;
  }
  return true;
}

void
surveillance_interfaces__action__SecurityAction_SendGoal_Request__fini(surveillance_interfaces__action__SecurityAction_SendGoal_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // goal
  surveillance_interfaces__action__SecurityAction_Goal__fini(&msg->goal);
}

bool
surveillance_interfaces__action__SecurityAction_SendGoal_Request__are_equal(const surveillance_interfaces__action__SecurityAction_SendGoal_Request * lhs, const surveillance_interfaces__action__SecurityAction_SendGoal_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // goal
  if (!surveillance_interfaces__action__SecurityAction_Goal__are_equal(
      &(lhs->goal), &(rhs->goal)))
  {
    return false;
  }
  return true;
}

bool
surveillance_interfaces__action__SecurityAction_SendGoal_Request__copy(
  const surveillance_interfaces__action__SecurityAction_SendGoal_Request * input,
  surveillance_interfaces__action__SecurityAction_SendGoal_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // goal
  if (!surveillance_interfaces__action__SecurityAction_Goal__copy(
      &(input->goal), &(output->goal)))
  {
    return false;
  }
  return true;
}

surveillance_interfaces__action__SecurityAction_SendGoal_Request *
surveillance_interfaces__action__SecurityAction_SendGoal_Request__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_SendGoal_Request * msg = (surveillance_interfaces__action__SecurityAction_SendGoal_Request *)allocator.allocate(sizeof(surveillance_interfaces__action__SecurityAction_SendGoal_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(surveillance_interfaces__action__SecurityAction_SendGoal_Request));
  bool success = surveillance_interfaces__action__SecurityAction_SendGoal_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
surveillance_interfaces__action__SecurityAction_SendGoal_Request__destroy(surveillance_interfaces__action__SecurityAction_SendGoal_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    surveillance_interfaces__action__SecurityAction_SendGoal_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
surveillance_interfaces__action__SecurityAction_SendGoal_Request__Sequence__init(surveillance_interfaces__action__SecurityAction_SendGoal_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_SendGoal_Request * data = NULL;

  if (size) {
    data = (surveillance_interfaces__action__SecurityAction_SendGoal_Request *)allocator.zero_allocate(size, sizeof(surveillance_interfaces__action__SecurityAction_SendGoal_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = surveillance_interfaces__action__SecurityAction_SendGoal_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        surveillance_interfaces__action__SecurityAction_SendGoal_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
surveillance_interfaces__action__SecurityAction_SendGoal_Request__Sequence__fini(surveillance_interfaces__action__SecurityAction_SendGoal_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      surveillance_interfaces__action__SecurityAction_SendGoal_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

surveillance_interfaces__action__SecurityAction_SendGoal_Request__Sequence *
surveillance_interfaces__action__SecurityAction_SendGoal_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_SendGoal_Request__Sequence * array = (surveillance_interfaces__action__SecurityAction_SendGoal_Request__Sequence *)allocator.allocate(sizeof(surveillance_interfaces__action__SecurityAction_SendGoal_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = surveillance_interfaces__action__SecurityAction_SendGoal_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
surveillance_interfaces__action__SecurityAction_SendGoal_Request__Sequence__destroy(surveillance_interfaces__action__SecurityAction_SendGoal_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    surveillance_interfaces__action__SecurityAction_SendGoal_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
surveillance_interfaces__action__SecurityAction_SendGoal_Request__Sequence__are_equal(const surveillance_interfaces__action__SecurityAction_SendGoal_Request__Sequence * lhs, const surveillance_interfaces__action__SecurityAction_SendGoal_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!surveillance_interfaces__action__SecurityAction_SendGoal_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
surveillance_interfaces__action__SecurityAction_SendGoal_Request__Sequence__copy(
  const surveillance_interfaces__action__SecurityAction_SendGoal_Request__Sequence * input,
  surveillance_interfaces__action__SecurityAction_SendGoal_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(surveillance_interfaces__action__SecurityAction_SendGoal_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    surveillance_interfaces__action__SecurityAction_SendGoal_Request * data =
      (surveillance_interfaces__action__SecurityAction_SendGoal_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!surveillance_interfaces__action__SecurityAction_SendGoal_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          surveillance_interfaces__action__SecurityAction_SendGoal_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!surveillance_interfaces__action__SecurityAction_SendGoal_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `stamp`
#include "builtin_interfaces/msg/detail/time__functions.h"

bool
surveillance_interfaces__action__SecurityAction_SendGoal_Response__init(surveillance_interfaces__action__SecurityAction_SendGoal_Response * msg)
{
  if (!msg) {
    return false;
  }
  // accepted
  // stamp
  if (!builtin_interfaces__msg__Time__init(&msg->stamp)) {
    surveillance_interfaces__action__SecurityAction_SendGoal_Response__fini(msg);
    return false;
  }
  return true;
}

void
surveillance_interfaces__action__SecurityAction_SendGoal_Response__fini(surveillance_interfaces__action__SecurityAction_SendGoal_Response * msg)
{
  if (!msg) {
    return;
  }
  // accepted
  // stamp
  builtin_interfaces__msg__Time__fini(&msg->stamp);
}

bool
surveillance_interfaces__action__SecurityAction_SendGoal_Response__are_equal(const surveillance_interfaces__action__SecurityAction_SendGoal_Response * lhs, const surveillance_interfaces__action__SecurityAction_SendGoal_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // accepted
  if (lhs->accepted != rhs->accepted) {
    return false;
  }
  // stamp
  if (!builtin_interfaces__msg__Time__are_equal(
      &(lhs->stamp), &(rhs->stamp)))
  {
    return false;
  }
  return true;
}

bool
surveillance_interfaces__action__SecurityAction_SendGoal_Response__copy(
  const surveillance_interfaces__action__SecurityAction_SendGoal_Response * input,
  surveillance_interfaces__action__SecurityAction_SendGoal_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // accepted
  output->accepted = input->accepted;
  // stamp
  if (!builtin_interfaces__msg__Time__copy(
      &(input->stamp), &(output->stamp)))
  {
    return false;
  }
  return true;
}

surveillance_interfaces__action__SecurityAction_SendGoal_Response *
surveillance_interfaces__action__SecurityAction_SendGoal_Response__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_SendGoal_Response * msg = (surveillance_interfaces__action__SecurityAction_SendGoal_Response *)allocator.allocate(sizeof(surveillance_interfaces__action__SecurityAction_SendGoal_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(surveillance_interfaces__action__SecurityAction_SendGoal_Response));
  bool success = surveillance_interfaces__action__SecurityAction_SendGoal_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
surveillance_interfaces__action__SecurityAction_SendGoal_Response__destroy(surveillance_interfaces__action__SecurityAction_SendGoal_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    surveillance_interfaces__action__SecurityAction_SendGoal_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
surveillance_interfaces__action__SecurityAction_SendGoal_Response__Sequence__init(surveillance_interfaces__action__SecurityAction_SendGoal_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_SendGoal_Response * data = NULL;

  if (size) {
    data = (surveillance_interfaces__action__SecurityAction_SendGoal_Response *)allocator.zero_allocate(size, sizeof(surveillance_interfaces__action__SecurityAction_SendGoal_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = surveillance_interfaces__action__SecurityAction_SendGoal_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        surveillance_interfaces__action__SecurityAction_SendGoal_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
surveillance_interfaces__action__SecurityAction_SendGoal_Response__Sequence__fini(surveillance_interfaces__action__SecurityAction_SendGoal_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      surveillance_interfaces__action__SecurityAction_SendGoal_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

surveillance_interfaces__action__SecurityAction_SendGoal_Response__Sequence *
surveillance_interfaces__action__SecurityAction_SendGoal_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_SendGoal_Response__Sequence * array = (surveillance_interfaces__action__SecurityAction_SendGoal_Response__Sequence *)allocator.allocate(sizeof(surveillance_interfaces__action__SecurityAction_SendGoal_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = surveillance_interfaces__action__SecurityAction_SendGoal_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
surveillance_interfaces__action__SecurityAction_SendGoal_Response__Sequence__destroy(surveillance_interfaces__action__SecurityAction_SendGoal_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    surveillance_interfaces__action__SecurityAction_SendGoal_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
surveillance_interfaces__action__SecurityAction_SendGoal_Response__Sequence__are_equal(const surveillance_interfaces__action__SecurityAction_SendGoal_Response__Sequence * lhs, const surveillance_interfaces__action__SecurityAction_SendGoal_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!surveillance_interfaces__action__SecurityAction_SendGoal_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
surveillance_interfaces__action__SecurityAction_SendGoal_Response__Sequence__copy(
  const surveillance_interfaces__action__SecurityAction_SendGoal_Response__Sequence * input,
  surveillance_interfaces__action__SecurityAction_SendGoal_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(surveillance_interfaces__action__SecurityAction_SendGoal_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    surveillance_interfaces__action__SecurityAction_SendGoal_Response * data =
      (surveillance_interfaces__action__SecurityAction_SendGoal_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!surveillance_interfaces__action__SecurityAction_SendGoal_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          surveillance_interfaces__action__SecurityAction_SendGoal_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!surveillance_interfaces__action__SecurityAction_SendGoal_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `info`
#include "service_msgs/msg/detail/service_event_info__functions.h"
// Member `request`
// Member `response`
// already included above
// #include "surveillance_interfaces/action/detail/security_action__functions.h"

bool
surveillance_interfaces__action__SecurityAction_SendGoal_Event__init(surveillance_interfaces__action__SecurityAction_SendGoal_Event * msg)
{
  if (!msg) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__init(&msg->info)) {
    surveillance_interfaces__action__SecurityAction_SendGoal_Event__fini(msg);
    return false;
  }
  // request
  if (!surveillance_interfaces__action__SecurityAction_SendGoal_Request__Sequence__init(&msg->request, 0)) {
    surveillance_interfaces__action__SecurityAction_SendGoal_Event__fini(msg);
    return false;
  }
  // response
  if (!surveillance_interfaces__action__SecurityAction_SendGoal_Response__Sequence__init(&msg->response, 0)) {
    surveillance_interfaces__action__SecurityAction_SendGoal_Event__fini(msg);
    return false;
  }
  return true;
}

void
surveillance_interfaces__action__SecurityAction_SendGoal_Event__fini(surveillance_interfaces__action__SecurityAction_SendGoal_Event * msg)
{
  if (!msg) {
    return;
  }
  // info
  service_msgs__msg__ServiceEventInfo__fini(&msg->info);
  // request
  surveillance_interfaces__action__SecurityAction_SendGoal_Request__Sequence__fini(&msg->request);
  // response
  surveillance_interfaces__action__SecurityAction_SendGoal_Response__Sequence__fini(&msg->response);
}

bool
surveillance_interfaces__action__SecurityAction_SendGoal_Event__are_equal(const surveillance_interfaces__action__SecurityAction_SendGoal_Event * lhs, const surveillance_interfaces__action__SecurityAction_SendGoal_Event * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__are_equal(
      &(lhs->info), &(rhs->info)))
  {
    return false;
  }
  // request
  if (!surveillance_interfaces__action__SecurityAction_SendGoal_Request__Sequence__are_equal(
      &(lhs->request), &(rhs->request)))
  {
    return false;
  }
  // response
  if (!surveillance_interfaces__action__SecurityAction_SendGoal_Response__Sequence__are_equal(
      &(lhs->response), &(rhs->response)))
  {
    return false;
  }
  return true;
}

bool
surveillance_interfaces__action__SecurityAction_SendGoal_Event__copy(
  const surveillance_interfaces__action__SecurityAction_SendGoal_Event * input,
  surveillance_interfaces__action__SecurityAction_SendGoal_Event * output)
{
  if (!input || !output) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__copy(
      &(input->info), &(output->info)))
  {
    return false;
  }
  // request
  if (!surveillance_interfaces__action__SecurityAction_SendGoal_Request__Sequence__copy(
      &(input->request), &(output->request)))
  {
    return false;
  }
  // response
  if (!surveillance_interfaces__action__SecurityAction_SendGoal_Response__Sequence__copy(
      &(input->response), &(output->response)))
  {
    return false;
  }
  return true;
}

surveillance_interfaces__action__SecurityAction_SendGoal_Event *
surveillance_interfaces__action__SecurityAction_SendGoal_Event__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_SendGoal_Event * msg = (surveillance_interfaces__action__SecurityAction_SendGoal_Event *)allocator.allocate(sizeof(surveillance_interfaces__action__SecurityAction_SendGoal_Event), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(surveillance_interfaces__action__SecurityAction_SendGoal_Event));
  bool success = surveillance_interfaces__action__SecurityAction_SendGoal_Event__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
surveillance_interfaces__action__SecurityAction_SendGoal_Event__destroy(surveillance_interfaces__action__SecurityAction_SendGoal_Event * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    surveillance_interfaces__action__SecurityAction_SendGoal_Event__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
surveillance_interfaces__action__SecurityAction_SendGoal_Event__Sequence__init(surveillance_interfaces__action__SecurityAction_SendGoal_Event__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_SendGoal_Event * data = NULL;

  if (size) {
    data = (surveillance_interfaces__action__SecurityAction_SendGoal_Event *)allocator.zero_allocate(size, sizeof(surveillance_interfaces__action__SecurityAction_SendGoal_Event), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = surveillance_interfaces__action__SecurityAction_SendGoal_Event__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        surveillance_interfaces__action__SecurityAction_SendGoal_Event__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
surveillance_interfaces__action__SecurityAction_SendGoal_Event__Sequence__fini(surveillance_interfaces__action__SecurityAction_SendGoal_Event__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      surveillance_interfaces__action__SecurityAction_SendGoal_Event__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

surveillance_interfaces__action__SecurityAction_SendGoal_Event__Sequence *
surveillance_interfaces__action__SecurityAction_SendGoal_Event__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_SendGoal_Event__Sequence * array = (surveillance_interfaces__action__SecurityAction_SendGoal_Event__Sequence *)allocator.allocate(sizeof(surveillance_interfaces__action__SecurityAction_SendGoal_Event__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = surveillance_interfaces__action__SecurityAction_SendGoal_Event__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
surveillance_interfaces__action__SecurityAction_SendGoal_Event__Sequence__destroy(surveillance_interfaces__action__SecurityAction_SendGoal_Event__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    surveillance_interfaces__action__SecurityAction_SendGoal_Event__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
surveillance_interfaces__action__SecurityAction_SendGoal_Event__Sequence__are_equal(const surveillance_interfaces__action__SecurityAction_SendGoal_Event__Sequence * lhs, const surveillance_interfaces__action__SecurityAction_SendGoal_Event__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!surveillance_interfaces__action__SecurityAction_SendGoal_Event__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
surveillance_interfaces__action__SecurityAction_SendGoal_Event__Sequence__copy(
  const surveillance_interfaces__action__SecurityAction_SendGoal_Event__Sequence * input,
  surveillance_interfaces__action__SecurityAction_SendGoal_Event__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(surveillance_interfaces__action__SecurityAction_SendGoal_Event);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    surveillance_interfaces__action__SecurityAction_SendGoal_Event * data =
      (surveillance_interfaces__action__SecurityAction_SendGoal_Event *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!surveillance_interfaces__action__SecurityAction_SendGoal_Event__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          surveillance_interfaces__action__SecurityAction_SendGoal_Event__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!surveillance_interfaces__action__SecurityAction_SendGoal_Event__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"

bool
surveillance_interfaces__action__SecurityAction_GetResult_Request__init(surveillance_interfaces__action__SecurityAction_GetResult_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    surveillance_interfaces__action__SecurityAction_GetResult_Request__fini(msg);
    return false;
  }
  return true;
}

void
surveillance_interfaces__action__SecurityAction_GetResult_Request__fini(surveillance_interfaces__action__SecurityAction_GetResult_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
}

bool
surveillance_interfaces__action__SecurityAction_GetResult_Request__are_equal(const surveillance_interfaces__action__SecurityAction_GetResult_Request * lhs, const surveillance_interfaces__action__SecurityAction_GetResult_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  return true;
}

bool
surveillance_interfaces__action__SecurityAction_GetResult_Request__copy(
  const surveillance_interfaces__action__SecurityAction_GetResult_Request * input,
  surveillance_interfaces__action__SecurityAction_GetResult_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  return true;
}

surveillance_interfaces__action__SecurityAction_GetResult_Request *
surveillance_interfaces__action__SecurityAction_GetResult_Request__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_GetResult_Request * msg = (surveillance_interfaces__action__SecurityAction_GetResult_Request *)allocator.allocate(sizeof(surveillance_interfaces__action__SecurityAction_GetResult_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(surveillance_interfaces__action__SecurityAction_GetResult_Request));
  bool success = surveillance_interfaces__action__SecurityAction_GetResult_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
surveillance_interfaces__action__SecurityAction_GetResult_Request__destroy(surveillance_interfaces__action__SecurityAction_GetResult_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    surveillance_interfaces__action__SecurityAction_GetResult_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
surveillance_interfaces__action__SecurityAction_GetResult_Request__Sequence__init(surveillance_interfaces__action__SecurityAction_GetResult_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_GetResult_Request * data = NULL;

  if (size) {
    data = (surveillance_interfaces__action__SecurityAction_GetResult_Request *)allocator.zero_allocate(size, sizeof(surveillance_interfaces__action__SecurityAction_GetResult_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = surveillance_interfaces__action__SecurityAction_GetResult_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        surveillance_interfaces__action__SecurityAction_GetResult_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
surveillance_interfaces__action__SecurityAction_GetResult_Request__Sequence__fini(surveillance_interfaces__action__SecurityAction_GetResult_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      surveillance_interfaces__action__SecurityAction_GetResult_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

surveillance_interfaces__action__SecurityAction_GetResult_Request__Sequence *
surveillance_interfaces__action__SecurityAction_GetResult_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_GetResult_Request__Sequence * array = (surveillance_interfaces__action__SecurityAction_GetResult_Request__Sequence *)allocator.allocate(sizeof(surveillance_interfaces__action__SecurityAction_GetResult_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = surveillance_interfaces__action__SecurityAction_GetResult_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
surveillance_interfaces__action__SecurityAction_GetResult_Request__Sequence__destroy(surveillance_interfaces__action__SecurityAction_GetResult_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    surveillance_interfaces__action__SecurityAction_GetResult_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
surveillance_interfaces__action__SecurityAction_GetResult_Request__Sequence__are_equal(const surveillance_interfaces__action__SecurityAction_GetResult_Request__Sequence * lhs, const surveillance_interfaces__action__SecurityAction_GetResult_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!surveillance_interfaces__action__SecurityAction_GetResult_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
surveillance_interfaces__action__SecurityAction_GetResult_Request__Sequence__copy(
  const surveillance_interfaces__action__SecurityAction_GetResult_Request__Sequence * input,
  surveillance_interfaces__action__SecurityAction_GetResult_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(surveillance_interfaces__action__SecurityAction_GetResult_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    surveillance_interfaces__action__SecurityAction_GetResult_Request * data =
      (surveillance_interfaces__action__SecurityAction_GetResult_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!surveillance_interfaces__action__SecurityAction_GetResult_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          surveillance_interfaces__action__SecurityAction_GetResult_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!surveillance_interfaces__action__SecurityAction_GetResult_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `result`
// already included above
// #include "surveillance_interfaces/action/detail/security_action__functions.h"

bool
surveillance_interfaces__action__SecurityAction_GetResult_Response__init(surveillance_interfaces__action__SecurityAction_GetResult_Response * msg)
{
  if (!msg) {
    return false;
  }
  // status
  // result
  if (!surveillance_interfaces__action__SecurityAction_Result__init(&msg->result)) {
    surveillance_interfaces__action__SecurityAction_GetResult_Response__fini(msg);
    return false;
  }
  return true;
}

void
surveillance_interfaces__action__SecurityAction_GetResult_Response__fini(surveillance_interfaces__action__SecurityAction_GetResult_Response * msg)
{
  if (!msg) {
    return;
  }
  // status
  // result
  surveillance_interfaces__action__SecurityAction_Result__fini(&msg->result);
}

bool
surveillance_interfaces__action__SecurityAction_GetResult_Response__are_equal(const surveillance_interfaces__action__SecurityAction_GetResult_Response * lhs, const surveillance_interfaces__action__SecurityAction_GetResult_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // status
  if (lhs->status != rhs->status) {
    return false;
  }
  // result
  if (!surveillance_interfaces__action__SecurityAction_Result__are_equal(
      &(lhs->result), &(rhs->result)))
  {
    return false;
  }
  return true;
}

bool
surveillance_interfaces__action__SecurityAction_GetResult_Response__copy(
  const surveillance_interfaces__action__SecurityAction_GetResult_Response * input,
  surveillance_interfaces__action__SecurityAction_GetResult_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // status
  output->status = input->status;
  // result
  if (!surveillance_interfaces__action__SecurityAction_Result__copy(
      &(input->result), &(output->result)))
  {
    return false;
  }
  return true;
}

surveillance_interfaces__action__SecurityAction_GetResult_Response *
surveillance_interfaces__action__SecurityAction_GetResult_Response__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_GetResult_Response * msg = (surveillance_interfaces__action__SecurityAction_GetResult_Response *)allocator.allocate(sizeof(surveillance_interfaces__action__SecurityAction_GetResult_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(surveillance_interfaces__action__SecurityAction_GetResult_Response));
  bool success = surveillance_interfaces__action__SecurityAction_GetResult_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
surveillance_interfaces__action__SecurityAction_GetResult_Response__destroy(surveillance_interfaces__action__SecurityAction_GetResult_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    surveillance_interfaces__action__SecurityAction_GetResult_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
surveillance_interfaces__action__SecurityAction_GetResult_Response__Sequence__init(surveillance_interfaces__action__SecurityAction_GetResult_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_GetResult_Response * data = NULL;

  if (size) {
    data = (surveillance_interfaces__action__SecurityAction_GetResult_Response *)allocator.zero_allocate(size, sizeof(surveillance_interfaces__action__SecurityAction_GetResult_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = surveillance_interfaces__action__SecurityAction_GetResult_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        surveillance_interfaces__action__SecurityAction_GetResult_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
surveillance_interfaces__action__SecurityAction_GetResult_Response__Sequence__fini(surveillance_interfaces__action__SecurityAction_GetResult_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      surveillance_interfaces__action__SecurityAction_GetResult_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

surveillance_interfaces__action__SecurityAction_GetResult_Response__Sequence *
surveillance_interfaces__action__SecurityAction_GetResult_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_GetResult_Response__Sequence * array = (surveillance_interfaces__action__SecurityAction_GetResult_Response__Sequence *)allocator.allocate(sizeof(surveillance_interfaces__action__SecurityAction_GetResult_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = surveillance_interfaces__action__SecurityAction_GetResult_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
surveillance_interfaces__action__SecurityAction_GetResult_Response__Sequence__destroy(surveillance_interfaces__action__SecurityAction_GetResult_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    surveillance_interfaces__action__SecurityAction_GetResult_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
surveillance_interfaces__action__SecurityAction_GetResult_Response__Sequence__are_equal(const surveillance_interfaces__action__SecurityAction_GetResult_Response__Sequence * lhs, const surveillance_interfaces__action__SecurityAction_GetResult_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!surveillance_interfaces__action__SecurityAction_GetResult_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
surveillance_interfaces__action__SecurityAction_GetResult_Response__Sequence__copy(
  const surveillance_interfaces__action__SecurityAction_GetResult_Response__Sequence * input,
  surveillance_interfaces__action__SecurityAction_GetResult_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(surveillance_interfaces__action__SecurityAction_GetResult_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    surveillance_interfaces__action__SecurityAction_GetResult_Response * data =
      (surveillance_interfaces__action__SecurityAction_GetResult_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!surveillance_interfaces__action__SecurityAction_GetResult_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          surveillance_interfaces__action__SecurityAction_GetResult_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!surveillance_interfaces__action__SecurityAction_GetResult_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `info`
// already included above
// #include "service_msgs/msg/detail/service_event_info__functions.h"
// Member `request`
// Member `response`
// already included above
// #include "surveillance_interfaces/action/detail/security_action__functions.h"

bool
surveillance_interfaces__action__SecurityAction_GetResult_Event__init(surveillance_interfaces__action__SecurityAction_GetResult_Event * msg)
{
  if (!msg) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__init(&msg->info)) {
    surveillance_interfaces__action__SecurityAction_GetResult_Event__fini(msg);
    return false;
  }
  // request
  if (!surveillance_interfaces__action__SecurityAction_GetResult_Request__Sequence__init(&msg->request, 0)) {
    surveillance_interfaces__action__SecurityAction_GetResult_Event__fini(msg);
    return false;
  }
  // response
  if (!surveillance_interfaces__action__SecurityAction_GetResult_Response__Sequence__init(&msg->response, 0)) {
    surveillance_interfaces__action__SecurityAction_GetResult_Event__fini(msg);
    return false;
  }
  return true;
}

void
surveillance_interfaces__action__SecurityAction_GetResult_Event__fini(surveillance_interfaces__action__SecurityAction_GetResult_Event * msg)
{
  if (!msg) {
    return;
  }
  // info
  service_msgs__msg__ServiceEventInfo__fini(&msg->info);
  // request
  surveillance_interfaces__action__SecurityAction_GetResult_Request__Sequence__fini(&msg->request);
  // response
  surveillance_interfaces__action__SecurityAction_GetResult_Response__Sequence__fini(&msg->response);
}

bool
surveillance_interfaces__action__SecurityAction_GetResult_Event__are_equal(const surveillance_interfaces__action__SecurityAction_GetResult_Event * lhs, const surveillance_interfaces__action__SecurityAction_GetResult_Event * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__are_equal(
      &(lhs->info), &(rhs->info)))
  {
    return false;
  }
  // request
  if (!surveillance_interfaces__action__SecurityAction_GetResult_Request__Sequence__are_equal(
      &(lhs->request), &(rhs->request)))
  {
    return false;
  }
  // response
  if (!surveillance_interfaces__action__SecurityAction_GetResult_Response__Sequence__are_equal(
      &(lhs->response), &(rhs->response)))
  {
    return false;
  }
  return true;
}

bool
surveillance_interfaces__action__SecurityAction_GetResult_Event__copy(
  const surveillance_interfaces__action__SecurityAction_GetResult_Event * input,
  surveillance_interfaces__action__SecurityAction_GetResult_Event * output)
{
  if (!input || !output) {
    return false;
  }
  // info
  if (!service_msgs__msg__ServiceEventInfo__copy(
      &(input->info), &(output->info)))
  {
    return false;
  }
  // request
  if (!surveillance_interfaces__action__SecurityAction_GetResult_Request__Sequence__copy(
      &(input->request), &(output->request)))
  {
    return false;
  }
  // response
  if (!surveillance_interfaces__action__SecurityAction_GetResult_Response__Sequence__copy(
      &(input->response), &(output->response)))
  {
    return false;
  }
  return true;
}

surveillance_interfaces__action__SecurityAction_GetResult_Event *
surveillance_interfaces__action__SecurityAction_GetResult_Event__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_GetResult_Event * msg = (surveillance_interfaces__action__SecurityAction_GetResult_Event *)allocator.allocate(sizeof(surveillance_interfaces__action__SecurityAction_GetResult_Event), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(surveillance_interfaces__action__SecurityAction_GetResult_Event));
  bool success = surveillance_interfaces__action__SecurityAction_GetResult_Event__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
surveillance_interfaces__action__SecurityAction_GetResult_Event__destroy(surveillance_interfaces__action__SecurityAction_GetResult_Event * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    surveillance_interfaces__action__SecurityAction_GetResult_Event__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
surveillance_interfaces__action__SecurityAction_GetResult_Event__Sequence__init(surveillance_interfaces__action__SecurityAction_GetResult_Event__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_GetResult_Event * data = NULL;

  if (size) {
    data = (surveillance_interfaces__action__SecurityAction_GetResult_Event *)allocator.zero_allocate(size, sizeof(surveillance_interfaces__action__SecurityAction_GetResult_Event), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = surveillance_interfaces__action__SecurityAction_GetResult_Event__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        surveillance_interfaces__action__SecurityAction_GetResult_Event__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
surveillance_interfaces__action__SecurityAction_GetResult_Event__Sequence__fini(surveillance_interfaces__action__SecurityAction_GetResult_Event__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      surveillance_interfaces__action__SecurityAction_GetResult_Event__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

surveillance_interfaces__action__SecurityAction_GetResult_Event__Sequence *
surveillance_interfaces__action__SecurityAction_GetResult_Event__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_GetResult_Event__Sequence * array = (surveillance_interfaces__action__SecurityAction_GetResult_Event__Sequence *)allocator.allocate(sizeof(surveillance_interfaces__action__SecurityAction_GetResult_Event__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = surveillance_interfaces__action__SecurityAction_GetResult_Event__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
surveillance_interfaces__action__SecurityAction_GetResult_Event__Sequence__destroy(surveillance_interfaces__action__SecurityAction_GetResult_Event__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    surveillance_interfaces__action__SecurityAction_GetResult_Event__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
surveillance_interfaces__action__SecurityAction_GetResult_Event__Sequence__are_equal(const surveillance_interfaces__action__SecurityAction_GetResult_Event__Sequence * lhs, const surveillance_interfaces__action__SecurityAction_GetResult_Event__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!surveillance_interfaces__action__SecurityAction_GetResult_Event__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
surveillance_interfaces__action__SecurityAction_GetResult_Event__Sequence__copy(
  const surveillance_interfaces__action__SecurityAction_GetResult_Event__Sequence * input,
  surveillance_interfaces__action__SecurityAction_GetResult_Event__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(surveillance_interfaces__action__SecurityAction_GetResult_Event);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    surveillance_interfaces__action__SecurityAction_GetResult_Event * data =
      (surveillance_interfaces__action__SecurityAction_GetResult_Event *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!surveillance_interfaces__action__SecurityAction_GetResult_Event__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          surveillance_interfaces__action__SecurityAction_GetResult_Event__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!surveillance_interfaces__action__SecurityAction_GetResult_Event__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `feedback`
// already included above
// #include "surveillance_interfaces/action/detail/security_action__functions.h"

bool
surveillance_interfaces__action__SecurityAction_FeedbackMessage__init(surveillance_interfaces__action__SecurityAction_FeedbackMessage * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    surveillance_interfaces__action__SecurityAction_FeedbackMessage__fini(msg);
    return false;
  }
  // feedback
  if (!surveillance_interfaces__action__SecurityAction_Feedback__init(&msg->feedback)) {
    surveillance_interfaces__action__SecurityAction_FeedbackMessage__fini(msg);
    return false;
  }
  return true;
}

void
surveillance_interfaces__action__SecurityAction_FeedbackMessage__fini(surveillance_interfaces__action__SecurityAction_FeedbackMessage * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // feedback
  surveillance_interfaces__action__SecurityAction_Feedback__fini(&msg->feedback);
}

bool
surveillance_interfaces__action__SecurityAction_FeedbackMessage__are_equal(const surveillance_interfaces__action__SecurityAction_FeedbackMessage * lhs, const surveillance_interfaces__action__SecurityAction_FeedbackMessage * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // feedback
  if (!surveillance_interfaces__action__SecurityAction_Feedback__are_equal(
      &(lhs->feedback), &(rhs->feedback)))
  {
    return false;
  }
  return true;
}

bool
surveillance_interfaces__action__SecurityAction_FeedbackMessage__copy(
  const surveillance_interfaces__action__SecurityAction_FeedbackMessage * input,
  surveillance_interfaces__action__SecurityAction_FeedbackMessage * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // feedback
  if (!surveillance_interfaces__action__SecurityAction_Feedback__copy(
      &(input->feedback), &(output->feedback)))
  {
    return false;
  }
  return true;
}

surveillance_interfaces__action__SecurityAction_FeedbackMessage *
surveillance_interfaces__action__SecurityAction_FeedbackMessage__create(void)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_FeedbackMessage * msg = (surveillance_interfaces__action__SecurityAction_FeedbackMessage *)allocator.allocate(sizeof(surveillance_interfaces__action__SecurityAction_FeedbackMessage), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(surveillance_interfaces__action__SecurityAction_FeedbackMessage));
  bool success = surveillance_interfaces__action__SecurityAction_FeedbackMessage__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
surveillance_interfaces__action__SecurityAction_FeedbackMessage__destroy(surveillance_interfaces__action__SecurityAction_FeedbackMessage * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    surveillance_interfaces__action__SecurityAction_FeedbackMessage__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
surveillance_interfaces__action__SecurityAction_FeedbackMessage__Sequence__init(surveillance_interfaces__action__SecurityAction_FeedbackMessage__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_FeedbackMessage * data = NULL;

  if (size) {
    data = (surveillance_interfaces__action__SecurityAction_FeedbackMessage *)allocator.zero_allocate(size, sizeof(surveillance_interfaces__action__SecurityAction_FeedbackMessage), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = surveillance_interfaces__action__SecurityAction_FeedbackMessage__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        surveillance_interfaces__action__SecurityAction_FeedbackMessage__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
surveillance_interfaces__action__SecurityAction_FeedbackMessage__Sequence__fini(surveillance_interfaces__action__SecurityAction_FeedbackMessage__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      surveillance_interfaces__action__SecurityAction_FeedbackMessage__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

surveillance_interfaces__action__SecurityAction_FeedbackMessage__Sequence *
surveillance_interfaces__action__SecurityAction_FeedbackMessage__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  surveillance_interfaces__action__SecurityAction_FeedbackMessage__Sequence * array = (surveillance_interfaces__action__SecurityAction_FeedbackMessage__Sequence *)allocator.allocate(sizeof(surveillance_interfaces__action__SecurityAction_FeedbackMessage__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = surveillance_interfaces__action__SecurityAction_FeedbackMessage__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
surveillance_interfaces__action__SecurityAction_FeedbackMessage__Sequence__destroy(surveillance_interfaces__action__SecurityAction_FeedbackMessage__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    surveillance_interfaces__action__SecurityAction_FeedbackMessage__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
surveillance_interfaces__action__SecurityAction_FeedbackMessage__Sequence__are_equal(const surveillance_interfaces__action__SecurityAction_FeedbackMessage__Sequence * lhs, const surveillance_interfaces__action__SecurityAction_FeedbackMessage__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!surveillance_interfaces__action__SecurityAction_FeedbackMessage__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
surveillance_interfaces__action__SecurityAction_FeedbackMessage__Sequence__copy(
  const surveillance_interfaces__action__SecurityAction_FeedbackMessage__Sequence * input,
  surveillance_interfaces__action__SecurityAction_FeedbackMessage__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(surveillance_interfaces__action__SecurityAction_FeedbackMessage);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    surveillance_interfaces__action__SecurityAction_FeedbackMessage * data =
      (surveillance_interfaces__action__SecurityAction_FeedbackMessage *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!surveillance_interfaces__action__SecurityAction_FeedbackMessage__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          surveillance_interfaces__action__SecurityAction_FeedbackMessage__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!surveillance_interfaces__action__SecurityAction_FeedbackMessage__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
